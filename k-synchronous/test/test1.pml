...
// assume N <= 8
// 01234567:1 bit to store matched field,
// 234 to store sid, 567 to store rid
#define TSENDER                 byte
#define SET_MATCHED(ts,matched) ts=ts|(matched<<6)
#define SET_SID(ts,sid)         ts=ts|(sid<<3)
#define SET_RID(ts,rid)         ts=ts|rid
#define IS_MATCHED(ts)          ts>>6
...
mtype = { Hello1, Hello2, Hello3, Hello4 }
...
inline changeFlag() {
  if
    :: empty(C1) && empty(C2) && empty(C3) && empty(C4) -> Mcausal(); flag!false 
    :: nempty(C1) || nempty(C2) || nempty(C3) || nempty(C4) -> flag!true
  fi
}
...
inline SendAction(_FROM, _TO, _ACT) {
  if
    ...
    :: _TO == P4ID -> SendActionToChannel(_FROM, _TO, C4, _ACT)
  fi
}
...
Send1:
  atomic{ SendActionToChannel(_pid, P4ID, C4, Hello1); goto Send2 }

Send2:
  atomic{ SendActionToChannel(_pid, P2ID, C2, Hello2)}
}

proctype P2() {
Rec:
  atomic{ flag?true ->
	  C2?Hello2 -> changeFlag(); goto Send}  
Send:
  atomic{ SendActionToChannel(_pid, P3ID, C3, Hello3) }
}

proctype P3() {
Rec1:
  atomic{ flag?true ->
	  C3?Hello3 -> changeFlag(); goto Rec2}
Rec2:
  atomic{ flag?true ->
	  C3?Hello4 -> changeFlag()} 
}

active proctype P4() {
  atomic{ flag!false; run P3(); run P2(); run P1(); run Pi() };
Send:
  atomic{ SendActionToChannel(_pid, P3ID, C3, Hello4); goto Rec }
Rec:
  atomic{ flag?true ->
	  C4?Hello1 -> changeFlag()}
}
/* Pi process */
proctype Pi() {
  mtype m;
