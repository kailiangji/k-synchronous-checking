...
/* changeFlag sets flag and flagPI to false while all the channels are
empty */
inline changeFlag() {
  if
    :: empty(C1) && empty(C2) -> Mcausal(); flag!false 
    :: nempty(C1) || nempty(C2) -> flag!true
  fi
}
...
Send1:
  SendActionToChannel(_pid, P2ID, C2, Hello1);
  goto Send2

Send2:
  SendActionToChannel(_pid, P2ID, C2, Hello2);
  goto Rec1
  
Rec1:
  atomic{flag?true ->
	 C1?Hello1 -> changeFlag()};
  goto Rec2

Rec2:
  atomic{flag?true ->
	 C1?Hello2 -> changeFlag()}
}

active proctype P2() {
  atomic{flag!false; run P1(); run Pi()};
  
Send1:
  SendActionToChannel(_pid, P1ID, C1, Hello1);
  goto Send2

Send2:
  SendActionToChannel(_pid, P1ID, C1, Hello2);
  goto Rec1
  
Rec1:
  atomic{flag?true ->
	 C2?Hello1 -> changeFlag()};
  goto Rec2

Rec2:
  atomic{flag?true ->
	 C2?Hello2 -> changeFlag()}
}
